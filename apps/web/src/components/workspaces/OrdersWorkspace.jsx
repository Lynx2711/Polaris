...
import { Package, Clock, CheckCircle, MoreVertical, ChevronLeft, ChevronRight } from 'lucide-react';
...
function StatusPill({ status }) {
  ...
  else if (s === 'COMPLETED') { color = 'var(--accent-green)'; bg = 'rgba(5,150,105,0.06)'; border = 'rgba(5,150,105,0.4)'; }
  return (
    <span style={{
...
export default function OrdersWorkspace({ orders = [], onAddOrder }) {
  ...
  const slice = orders.slice((page - 1) * PER, page * PER);

  return (
    <WorkspaceContainer id="orders" title="Order Management">
      {/* Controls */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginBottom: 28 }}>
        <button style={{ padding: '9px 20px', border: '1px solid var(--border)', background: 'none', cursor: 'pointer', borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--ink)', fontFamily: 'Inter,sans-serif', transition: 'background 0.15s' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 28 }}>
        {[
          { label: 'Total Orders', value: total, icon: <Package size={20}/>, bg: 'var(--surface-raised)', ic: 'var(--ink-muted)' },
        ...
        {selected && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 10, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
              <button style={{ flex: 1, padding: '12px 0', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--surface-raised)', color: 'var(--ink)', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'Inter,sans-serif' }}>Edit Order</button>
              <button style={{ flex: 1, padding: '12px 0', borderRadius: 10, border: 'none', background: 'var(--ink)', color: 'var(--surface)', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'Inter,sans-serif' }}>Assign Driver</button>
            </div>
          </div>
        )}
      </SlideOverDrawer>
