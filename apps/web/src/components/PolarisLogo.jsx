...
export function PolarisLogo({ size = 32, dark = false }) {
  ...
  useEffect(() => {
    const introTimer = setTimeout(() => {
      playArrowAnimation();
      setTimeout(() => setShowWord(true), 400); // Text slides out mid-spin
    }, 600);
  ...
  return (
    <div className="lockup">
      <div className="markSlide">
        <div className={`markSpin ${isAnimating ? 'animate' : ''}`} style={{ width: size, height: size }}>
      ...
      <div className={`word-clip ${showWord ? 'word-visible' : ''}`}>
        <span className={`word ${dark ? 'word-light' : ''}`}>
          POLARIS
        </span>
      </div>
    </div>
