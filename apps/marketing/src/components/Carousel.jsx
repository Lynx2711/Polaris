    ...
    >
      {item.image ? (
        <div className="w-full h-full relative flex flex-col justify-end">
          ...
          />
          {item.caption && (
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-4 pt-12 text-left z-10">
              <p className="text-sm font-semibold text-white m-0">{item.caption}</p>
            </div>
          )}
        </div>
      ) : (
